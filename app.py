...
# -----------------------------------------------------------------------------
    ...
    xp       = st.session_state["xp"]
    earned   = st.session_state["achievements"]
    visited  = st.session_state["monuments_visited"]
    ...
    st.markdown(
        f"""
        <div style="background:linear-gradient(135deg,#1F1F35,#242438);border:1px solid rgba(201,168,76,0.4);
        border-radius:16px;padding:2rem;margin-bottom:1.5rem;">
          <div style="font-family:'DM Mono',monospace;font-size:0.7rem;color:#7A6E5C;
               letter-spacing:0.15em;text-transform:uppercase;margin-bottom:0.4rem;">{T("current_level")}</div>
          <div style="font-family:'Cormorant Garamond',serif;font-size:2.2rem;
               color:#C9A84C;font-weight:600;">{level}</div>
          <div style="margin:1rem 0 0.4rem 0;background:rgba(255,255,255,0.06);
               border-radius:999px;height:10px;overflow:hidden;">
            <div style="width:{progress_pct}%;height:100%;
                 background:linear-gradient(90deg,#C9A84C,#E8C97A);border-radius:999px;
                 transition:width 0.5s ease;"></div>
          </div>
          <div style="display:flex;justify-content:space-between;font-size:0.8rem;color:#7A6E5C;">
            <span>⚡ {xp} {T("xp_earned")}</span>
            <span>{"Max level!" if not next_xp else f"{next_xp - xp} {T('xp_to_next')}"}</span>
          </div>
        </div>
        """,
        unsafe_allow_html=True,
    ...
    with s2:
        st.metric("🏛️ Monuments Visited", len(visited))
    with s3:
        st.metric("🧠 Quizzes Completed", quizzes)
    with s4:
        st.metric("🏅 Badges Earned", len(earned))

    st.markdown("---")
    st.markdown(f"### {T('badge_collection')}")

    UNLOCK_HINTS = {
        "first_visit":       ("Visit any monument", "🔍 Go to Monument Recognition and explore one"),
        "quiz_master":       ("Complete 2 quizzes",  "🧠 Go to Quiz and complete 2 monument quizzes"),
        "heritage_champion": ("Earn 150+ XP",        "⚡ Explore monuments, chat & take quizzes"),
    }

    # Show all 3 badges as big cards
    b1, b2, b3 = st.columns(3)
    for col, (key, (icon, name, desc)) in zip([b1, b2, b3], BADGES.items()):
        is_earned = key in earned
        hint, how  = UNLOCK_HINTS[key]
        with col:
            if is_earned:
                st.markdown(
                    f"""
                    <div style="background:linear-gradient(135deg,rgba(201,168,76,0.15),rgba(201,168,76,0.05));
                    border:2px solid rgba(201,168,76,0.6);border-radius:16px;padding:1.8rem 1.2rem;
                    text-align:center;height:100%;">
                      ...
                      <div style="font-family:'Cormorant Garamond',serif;font-size:1.2rem;
                           color:#E8C97A;font-weight:600;margin-bottom:0.4rem;">{name}</div>
                      <div style="font-size:0.82rem;color:#B8A98A;margin-bottom:1rem;">{desc}</div>
                      <div style="background:rgba(75,142,110,0.2);border:1px solid rgba(75,142,110,0.5);
                           border-radius:999px;padding:0.3rem 0.8rem;font-size:0.72rem;
                           color:#7ECDA0;display:inline-block;">{T("unlocked")}</div>
                    </div>
                    """,
                    unsafe_allow_html=True,
                )
                ...
                st.markdown(
                    ...
                    border-radius:16px;padding:1.8rem 1.2rem;text-align:center;height:100%;opacity:0.6;">
                      <div style="font-size:3rem;margin-bottom:0.8rem;filter:grayscale(1);">🔒</div>
                      <div style="font-family:'Cormorant Garamond',serif;font-size:1.2rem;
                           color:#7A6E5C;font-weight:600;margin-bottom:0.4rem;">{name}</div>
                      <div style="font-size:0.82rem;color:#5C5445;margin-bottom:0.6rem;">{hint}</div>
                      <div style="font-size:0.75rem;color:#4A4035;font-style:italic;">{how}</div>
                    </div>
                    """,
                    unsafe_allow_html=True,
                )
    ...
    # ── Monuments visited list ────────────────────────────────────────────────
    st.markdown(f"### {T('monuments_explored')}")
    if visited:
        all_monuments = get_all_monument_names()
        cols = st.columns(3)
        for i, m in enumerate(all_monuments):
            with cols[i % 3]:
                if m in visited:
                    st.markdown(
                        f'<div style="background:rgba(201,168,76,0.1);border:1px solid rgba(201,168,76,0.4);'
                        f'border-radius:8px;padding:0.6rem 1rem;margin:4px 0;color:#E8C97A;font-size:0.9rem;">'
                        f'✅ {m}</div>',
                        unsafe_allow_html=True,
                    )
                else:
                    st.markdown(
                        ...
                        f'border-radius:8px;padding:0.6rem 1rem;margin:4px 0;color:#5C5445;font-size:0.9rem;">'
                        f'⬜ {m}</div>',
                        unsafe_allow_html=True,
                    )
    else:
        st.info(T("no_monuments_yet"))

    # ── XP activity log ──────────────────────────────────────────────────────
    xp_log = st.session_state.get("xp_log", [])
    if xp_log:
        st.markdown("---")
        st.markdown(f"### {T('xp_activity_log')}")
        for entry in reversed(xp_log[-10:]):   # show last 10
            st.markdown(
                f'<div style="font-family:\'DM Mono\',monospace;font-size:0.78rem;'
                f'color:#C9A84C;padding:2px 0;">▸ {entry}</div>',
                unsafe_allow_html=True,
            )
